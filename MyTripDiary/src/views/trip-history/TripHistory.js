...
export default function ({ navigation }) {
  ...
  const [groupedTrips, setGroupedTrips] = useState(
    groupExecutedTripsByDate(executedTrips)
  );
  const [popupState, setPopupState] = useState({
    selectedExecutedTrip: null,
    visible: false,
  });
  ...
  useEffect(() => {
    const handleEvent = () => {
      setGroupedTrips(
        groupExecutedTripsByDate([...getExecutedTripsSortedByDate()])
      );
    };
  ...
  const handlePriceChange = () => {
    if (price == null || isNaN(price) || (!isNaN(price) && Number(price) < 0)) {
      Alert.alert(`Invalid Input`, `Please input the new price!`);
      return;
    }
  ...
  const renderSectionHeader = ({ section }) => {
    return (
      <Text fontWeight="bold" style={styles.sectionHeader}>
        {" "}
        {section.title}
      </Text>
    );
  ...
  const renderTrip = ({ item }) => {
    ...
    return (
      <TouchableComponent
        ...
        onPress={() => {
          navigation.navigate("ExecutedTripInfo", { item, trip });
        }}
        ...
        <Section style={styles.section}>
          ...
          <SectionContent>
            <View style={{ justifyContent: "center", alignItems: "center" }}>
              <Text>
                {getDisplayPrice(item)} | {startTime} - {endTime} |{" "}
                {item.distance.toFixed(1)}km by {item.modeOfTransport}
              </Text>
            </View>
          </SectionContent>
        </Section>
  ...
  return (
    <Layout>
      <TopNav middleContent="Trip History" />
      <SectionList
...
export function getCorrectPrice(execTripObj) {
  if (
    execTripObj.tripPrice.userInputPrice &&
    execTripObj.tripPrice.userInputPrice != -1
  ) {
